...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        const toggleButton = document.querySelector('button span:contains("View Package Details")')?.parentElement;

        if (toggleButton) {
            toggleButton.addEventListener('click', function () {
                console.log('Toggle button clicked');
                const isExpanded = toggleButton.getAttribute('data-expanded') === 'true';
                const newButtonText = isExpanded ? 'View Package Details' : 'Collapse Package Details';

                // Toggle class on all pricing card body and footer elements
                document.querySelectorAll('.pricing-card-body, .pricing-card-footer').forEach(el => {
                    el.classList.toggle('hidden', isExpanded); // hide if expanded, show otherwise
                });

                // Update button text
                toggleButton.querySelector('span').textContent = newButtonText;
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        // Ensure step2 starts hidden (in case HTML didn't include .hidden)
        hideStep(step2);
        showStep(step1);
        ...
        nextBtn.addEventListener('click', () => {
            ...
            if (clearBtn) {
                clearBtn.addEventListener('click', () => {
                    // your clear logic here (e.g., clear selections)
                    hideStep(step2);
                    showStep(step1);
                });
            }
