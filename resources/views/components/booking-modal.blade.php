{{-- Booking Form Modal --}}
<div class="modal px-3 sm:px-5" id="pricingModal">
    <div class="modal-content relative h-[calc(100vh-50px)] sm:h-[calc(100vh-100px)] w-full max-w-[751px] mx-auto">
        <form id="pricingBookingForm" class="h-full" method="POST"
            action="{{ route('booking.store', ['locale' => app()->getLocale()]) }}">
            @csrf

            <!-- Close Button -->
            <div class="close-modal-2 bg-white w-[28px] h-[28px] rounded-full flex items-center justify-center absolute top-[-14px] right-[-14px] z-50 cursor-pointer shadow-lg"
                onClick="closeModal()">
            ...
            <div class="h-full flex flex-col justify-between overflow-y-auto pr-2 sm:pr-5 md:pr-[30px] pl-2 sm:pl-5 py-4 sm:py-6">

                <!-- Success/Error Message -->
                <div id="formMessage" class="hidden mb-4 p-4 rounded-lg"></div>

                <!-- Header -->
                <div class="flex items-center gap-3 sm:gap-4 px-3 sm:px-5 py-4 sm:py-5 border border-[#C8CEDD] rounded-[12px] sm:rounded-[16px]">
                    <div
                        class="w-[40px] h-[40px] sm:w-[50px] sm:h-[50px] rounded-[12px] sm:rounded-[16px] bg-[#E7F1FF] flex items-center justify-center flex-shrink-0">
                        <svg width="20" height="20" class="sm:w-[26px] sm:h-[26px]" viewBox="0 0 26 26" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path
                    ...
                    <div class="space-y-1 sm:space-y-2">
                        <h4 class="text-[18px] sm:text-[24px] font-bold text-[var(--color-heading)] leading-[1.4]">
                            {{ __('Book your service') }}
                        </h4>
                        <p class="text-[14px] sm:text-[16px] font-medium text-[var(--color-text)] leading-[1.4] font-sans">
                            {{ __('For your') }}
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
                    ...
                    <div>
                        ...
                        <select id="packageSelect" name="package" required
                            class="block w-full px-3 sm:px-4 py-3 text-base text-gray-900 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-[var(--color-brand)] focus:ring-offset-2 focus:border-[var(--color-brand)]">
                            ...
                            <option value="Platinum Subscription - €274,45">{{ __('Platinum Subscription - €274,45') }}
                            </option>
