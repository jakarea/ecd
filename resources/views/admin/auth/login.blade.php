...
<body class="min-h-screen flex items-center justify-center p-4 sm:p-6 lg:p-8 bg-gradient-to-br from-[#6adbd9] to-[#124846]">
    <div class="bg-[#d5d5d5] rounded-2xl shadow-2xl w-full max-w-[480px] p-6 sm:p-8 lg:p-10">
        {{-- Logo Section --}}
        ...
        <form action="{{ route('admin.login.post', ['locale' => request()->route('locale') ?? request()->segment(1)]) }}" method="POST" class="space-y-5">
            ...
            <div>
                ...
                <input type="email" id="email" name="email"
                    class="w-full px-4 py-3 sm:py-3.5 border border-dark rounded-lg text-sm sm:text-base transition-all duration-200 focus:outline-none focus:border-[var(--color-brand)] focus:ring-2 focus:ring-[var(--color-brand)] focus:ring-opacity-20"
        ...
        <div class="text-center mt-6 sm:mt-8">
            <a href="{{ route('home', ['locale' => request()->route('locale') ?? request()->segment(1)]) }}"
                class="text-dark text-sm sm:text-base font-medium hover:underline inline-flex items-center gap-1.5 transition-all duration-200">
